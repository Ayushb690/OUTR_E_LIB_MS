...
export const register = catchAsyncErrors(async (req, res, next) => {
    ...
    try {
        ...
        if (registerationAttemptsByUser.length >= 10) {
            return next(new ErrorHandler("You have exceeded the maximum limit.please Contact the Support", 400));
        }
        ...
        console.log("USER CREATED 👉", user);   // 👈 ADD HERE
...
export const verifyOTP = catchAsyncErrors(async (req, res, next) => {
    ...
    try {
        const userAllEntries = await User.find({ email: normalizedEmail }).sort({ createdAt: -1 });
        console.log("DB RESULT 👉", userAllEntries);
        ...
        const userVerificationCodeExpire = new Date(user.verificationCodeExpire).getTime();

        if (currentTime > userVerificationCodeExpire) {
...
export const login = catchAsyncErrors(async (req, res, next) => {
    ...
    console.log("PASSWORD MATCH 👉", isPasswordMatched);  // 👈 ADD HERE
...
export const updatePassword = catchAsyncErrors(async (req, res, next) => {
    ...
    if (!isPasswordMatched) {
        return next(new ErrorHandler("Current passowrd is incorrect.", 400));
    }
